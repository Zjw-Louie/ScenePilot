...
def build_labeled_scene_summary(scene: Dict[str, Any]) -> Tuple[str, List[Dict[str, Any]]]:
    ...
    for i, obj in enumerate(objs):
        ...
        lines.append(f"  - {label} | pos=[{pos[0]:.3f}, {pos[1]:.3f}, {pos[2]:.3f}] | yaw={yaw:.1f}°")
...
1. Fix out-of-bounds
2. Fix collisions
3. Preserve clear functional grouping around dominant anchors
4. Keep interactive objects reachable with visible clearance
5. Prefer wall affinity for wall-side furniture, but avoid over-correcting if that hurts the whole layout

Important reasoning style:
- Think in terms of dominant anchors and accessories.
- Dominant anchors are large functional objects such as beds, desks, tables, sofas, counters, sinks, toilets, wardrobes, cabinets, and appliances.
- Accessories are smaller supporting objects such as chairs, nightstands, lamps, mirrors, rugs, and decor.
- When uncertain, keep dominant anchors stable and adjust accessories first.
- Avoid crowding many accessories around the same anchor unless it is clearly needed.
...
- Prefer fewer, higher-confidence edits.
...
"""
    ...
    def _chat(
        ...
        *,
        ...
        data = _post_chat_completions(self.api_base, self.api_key, payload, self.timeout_s, session=self.session)
