...
def main(args):
	...
	if args.test_ckpt is not None:
		...
		elif args.llm == "qwen-3-8B":
			model_id = "/home2/zhangjiawei/respace/model/qwen3-8B"
		elif args.llm == "qwen-3.5-4B":
			model_id = "/home2/zhangjiawei/respace/model/qwen3.5-4B"
		elif args.llm == "qwen-3-4B":
			model_id = "/home2/zhangjiawei/respace/model/qwen3-4B"
		else:
			raise ValueError("model not found")

	model, tokenizer, max_seq_length = get_model(model_id, args.use_gpu, accelerator)
	model, tokenizer, max_seq_length = get_model(
		model_id,
		args.use_gpu,
		accelerator,
		do_not_load_hf_model=False,
	)
...
if __name__ == "__main__":
	...
	parser.add_argument('--llm', type=str, choices=["llama-3.2-1B", "llama-3.2-3B", "llama-3.2-7B", "qwen-2.5-0.5B", "qwen-2.5-1.5B", "qwen-3-8B", "qwen-3.5-4B", "qwen-3-4B"], default="llama-3.2-1B")
