...
def _normalize_text_field(x):
    ...
    if isinstance(x, dict):
        # 优先常见文本字段
        for k in ["desc", "title", "name", "caption", "text"]:
            if k in x:
                return _normalize_text_field(x[k])
        try:
    ...
    if isinstance(x, (list, tuple)):
        parts = []
        for v in x:
            s = _normalize_text_field(v)
            if s:
                parts.append(s)
        return ", ".join(parts).strip()
    return str(x).strip()

def _normalize_numeric_list(x, length=None, default=None):
    if default is None:
        default = []
...
def _normalize_object_fields(obj):
    if not isinstance(obj, dict):
        return obj

    # 统一 desc
    if "desc" in obj:
        obj["desc"] = _normalize_text_field(obj["desc"])
    elif "title" in obj:
        obj["desc"] = _normalize_text_field(obj["title"])
    elif "name" in obj:
        obj["desc"] = _normalize_text_field(obj["name"])

    if "title" in obj:
        obj["title"] = _normalize_text_field(obj["title"])
    if "name" in obj:
        obj["name"] = _normalize_text_field(obj["name"])

    if "size" in obj:
        obj["size"] = _normalize_numeric_list(obj["size"], length=3, default=[1.0, 1.0, 1.0])
    if "pos" in obj:
        obj["pos"] = _normalize_numeric_list(obj["pos"], length=3, default=[0.0, 0.0, 0.0])
    if "rot" in obj:
        obj["rot"] = _normalize_numeric_list(obj["rot"], length=4, default=[0.0, 0.0, 0.0, 1.0])
...
class AssetRetrievalModule(nn.Module):
    def __init__(self, lambd, sigma, temp, top_p, top_k, asset_size_threshold, rand_seed=None, accelerator=None, dvc=None, do_print=False, is_sft_training=False):
        ...
        siglip_local_path = "/data/zhangjiawei/siglip-so400m-patch14-384"

        self.siglip_model = SiglipTextModel.from_pretrained(siglip_local_path, local_files_only=True)
        self.siglip_tokenizer = AutoTokenizer.from_pretrained(siglip_local_path, local_files_only=True)
    ...
    def compute_semantic_similarities(self, embeds):
        if embeds is None:
            print(f"idx [{self.accelerator.process_index if self.accelerator else None}] — query_embeds is None")
            return None
        ...
        if not torch.is_tensor(embeds):
            embeds = torch.tensor(
                embeds,
                dtype=torch.float32,
                device=(self.accelerator.device if self.accelerator else self.dvc),
            )

        if embeds.ndim == 1:
            embeds = embeds.unsqueeze(0)

        embeds = embeds.to(self.all_embeds_catalog.device)
        embeds_norm = torch.nn.functional.normalize(embeds, p=2, dim=1)
        semantic_sims = torch.matmul(embeds_norm, self.all_embeds_catalog.T)
    ...
    def forward_batch(self, descs, sizes):
        if not isinstance(descs, list):
            descs = [descs]
        descs = [_normalize_text_field(x) for x in descs]
        descs = [d if d else "object" for d in descs]

        if not isinstance(sizes, list):
            sizes = [sizes]
        sizes = [_normalize_numeric_list(s, length=3, default=[1.0, 1.0, 1.0]) for s in sizes]

        query_embeds = self.get_text_embeddings(descs)
        ...
        query_sizes = torch.tensor(
            sizes,
            dtype=torch.float32,
            device=self.all_sizes_catalog.device,
        )
        if query_sizes.ndim == 1:
            query_sizes = query_sizes.unsqueeze(0)

        size_sims = self.compute_size_similarities(query_sizes)

        sims_batch = self.lambd * semantic_sims.T + (1.0 - self.lambd) * size_sims
        probs = self.compute_final_probabilities(sims_batch)
        return probs
        ...
        if self.do_print:
            ...
            for idx, jid in zip(idxs_top, jids):
                ...
                if asset is None:
                    ...
                    size_sampled_obj = asset.get("size") if asset is not None else obj.get("size", [1.0, 1.0, 1.0])

                    orig_jid = asset.get("jid") if asset is not None else None
                    orig_asset = self.all_assets_metadata.get(orig_jid) if orig_jid is not None else None

                    desc_sampled_obj = (
                        (orig_asset.get("summary") if orig_asset is not None else None)
                        or (asset.get("summary") if asset is not None else None)
                        or obj.get("desc")
                        or obj.get("title")
                        or obj.get("name")
                        or "object"
                    ...
                    size_sampled_obj = asset.get("size") or obj.get("size", [1.0, 1.0, 1.0])

                desc_sampled_obj = _normalize_text_field(desc_sampled_obj)
                print(f"")
    ...
    def sample_last_asset(self, scene, is_greedy_sampling=False):
        if scene is None or "objects" not in scene or not scene["objects"]:
            return scene

        scene = copy.deepcopy(scene)
        scene["objects"][-1] = _normalize_object_fields(scene["objects"][-1])
        last_obj = scene["objects"][-1]

        desc = _normalize_text_field(
            last_obj.get("desc") or last_obj.get("title") or last_obj.get("name") or "object"
        )
        size = _normalize_numeric_list(last_obj.get("size"), length=3, default=[1.0, 1.0, 1.0])

        probs_batch = self.forward_batch([desc], [size])
        if probs_batch is None:
            print(f"idx [{self.accelerator.process_index if self.accelerator else None}] — asset retrieval skipped because probs is None")
            return scene

        probs = probs_batch[0] if probs_batch.ndim == 2 else probs_batch
        new_obj = self.create_sampled_obj(last_obj, probs, is_greedy_sampling)
        scene["objects"][-1] = new_obj
        return scene
