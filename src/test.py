...
GENERATION_RESERVE_TOKENS = 150


def get_batch_input_ids(queries, tokenizer, max_seq_length=None, reserve_new_tokens=GENERATION_RESERVE_TOKENS):
	batch_messages = [
	...
	try:
		...
		if max_seq_length is not None:
			max_prompt_tokens = max(1, max_seq_length - reserve_new_tokens)
...
def trim_left_for_generation_budget(input_ids, attention_mask, max_seq_length, reserve_new_tokens=GENERATION_RESERVE_TOKENS):
	max_prompt_tokens = max(1, max_seq_length - reserve_new_tokens)
...
def get_sample_outputs_batch(batch_instrs, model, tokenizer, max_seq_length, accelerator, n_best_of_n_llm, return_logits=False, temp=None, vllm_engine=None):
	...
	batch_input_ids, batch_attention_masks = get_batch_input_ids(
		batch_instrs,
		tokenizer,
		max_seq_length=max_seq_length,
		reserve_new_tokens=GENERATION_RESERVE_TOKENS,
	)
	...
	all_input_ids, all_attention_masks = trim_left_for_generation_budget(
		all_input_ids,
		all_attention_masks,
		max_seq_length=max_seq_length,
		reserve_new_tokens=GENERATION_RESERVE_TOKENS,
	...
	max_context = all_input_ids.shape[-1]
	max_tokens = max(1, min(max_seq_length - max_context, GENERATION_RESERVE_TOKENS))
	...
	print(
		f"\nnucleus sampling... (started: {readable_time}).. "
		f"batch_input_ids.shape: {batch_input_ids.shape} trimmed_context: {tuple(all_input_ids.shape)} max_new_tokens: {max_tokens}"
	)
...
def _is_complete_scene_dict(scene):
	if not isinstance(scene, dict):
		return False
	objects = scene.get("objects")
	bounds_bottom = scene.get("bounds_bottom")
	bounds_top = scene.get("bounds_top")
	return isinstance(objects, list) and bounds_bottom is not None and bounds_top is not None


def _ensure_full_scene_for_addition(scene_after, scene_before_with_assets):
	if _is_complete_scene_dict(scene_after):
		return scene_after
	merged_scene = create_full_scene_from_before_and_added(scene_before_with_assets, scene_after)
	if not _is_complete_scene_dict(merged_scene):
		raise ValueError(
			f"Merged scene is still incomplete. keys={list(merged_scene.keys()) if isinstance(merged_scene, dict) else type(merged_scene)}"
		)
	return merged_scene
...
def _flatten_text_value(value):
	...
	if isinstance(value, dict):
		for key in ["desc", "title", "name", "prompt", "caption", "text"]:
			if key in value:
				text = _flatten_text_value(value.get(key))
				if text:
					return text
		parts = []
		for v in value.values():
			text = _flatten_text_value(v)
			if text:
				parts.append(text)
...
def _to_float_list(value, expected_len=None, default=None):
	if isinstance(value, np.ndarray):
		value = value.tolist()
	if not isinstance(value, (list, tuple)):
		return copy.deepcopy(default) if default is not None else value
	flat = []
	for item in value:
	...
	if expected_len is not None and len(flat) != expected_len:
		return copy.deepcopy(default) if default is not None else flat
	return flat


def _normalize_generated_object(obj, prompt=None):
	if not isinstance(obj, dict):
		return obj
	obj = copy.deepcopy(obj)
	desc = _flatten_text_value(obj.get("desc"))
	if not desc:
		desc = _flatten_text_value(obj.get("title"))
	if not desc:
		desc = _flatten_text_value(obj.get("name"))
	if not desc:
		desc = _flatten_text_value(prompt)
	if desc:
		obj["desc"] = desc
	if "title" in obj and not isinstance(obj.get("title"), str):
		title = _flatten_text_value(obj.get("title"))
		if title:
			obj["title"] = title
	default_pos = [0.0, 0.0, 0.0]
	default_rot = [0.0, 0.0, 0.0, 1.0]
	default_size = [1.0, 1.0, 1.0]
	obj["pos"] = _to_float_list(obj.get("pos"), expected_len=3, default=default_pos)
	obj["rot"] = _to_float_list(obj.get("rot"), expected_len=4, default=default_rot)
	obj["size"] = _to_float_list(obj.get("size"), expected_len=3, default=default_size)
	return obj


def _normalize_generated_scene_for_addition(scene_after, prompt=None):
	if not isinstance(scene_after, dict):
		return scene_after
	scene_after = copy.deepcopy(scene_after)
	objects = scene_after.get("objects")
	if isinstance(objects, list) and len(objects) > 0:
		scene_after["objects"] = [_normalize_generated_object(obj, prompt=prompt) for obj in objects]
	else:
		for key in ["desc", "title", "name", "pos", "rot", "size"]:
			if key in scene_after:
				scene_after = _normalize_generated_object(scene_after, prompt=prompt)
				break
	return scene_after
...
def run_test_for_addition(scene_after, scene_before_with_assets, n_best_of_n_assets, sampling_engine=None):
	...
	scene_after = _normalize_generated_scene_for_addition(scene_after)
	scene_after = _ensure_full_scene_for_addition(scene_after, scene_before_with_assets)
	if isinstance(scene_after.get("objects"), list) and len(scene_after["objects"]) > 0:
		scene_after["objects"][-1] = _normalize_generated_object(scene_after["objects"][-1], prompt=scene_after["objects"][-1].get("prompt"))
	...
	for _ in range(n_best_of_n_assets):
		...
		if not _is_complete_scene_dict(scene_after_with_assets):
			raise ValueError(
				f"scene_after_with_assets is incomplete before eval. keys={list(scene_after_with_assets.keys()) if isinstance(scene_after_with_assets, dict) else type(scene_after_with_assets)}"
			)
...
def run_bon_test_for_addition(prompt, responses, scene_before_with_assets, n_best_of_n_assets, sampling_engine, pth_viz_output=None, filename=None, do_rendering_with_object_count=False, do_renderings=False):
	...
	for idx, response in enumerate(responses):
		...
		if scene_after:
			try:
				scene_after = _normalize_generated_scene_for_addition(scene_after, prompt=prompt)

				# add prompt to last object
				if scene_after.get("objects") is None:
					scene_after["prompt"] = prompt
				else:
					if len(scene_after["objects"]) == 0:
						raise ValueError("scene_after['objects'] is empty after parsing")
